// ================================
	...
	if (hexColorOrNull) {
	  document.body.style.background = hexToRgba(hexColorOrNull, 0.6);
	...
	function renderFregTable(act, pf){
      ...
      function aggregateTotais(pf){
        return pf.reduce((a,r)=>{ a.ad+=N(r.votos_ad); a.ps+=N(r.votos_ps); a.chega+=N(r.votos_chega); a.cdu+=N(r.votos_cdu); a.br+=N(r.brancos); a.nu+=N(r.nulos); return a; }, {ad:0,ps:0,chega:0,cdu:0,br:0,nu:0});
      }
	...
	function paintMap(act){
	  ...
	  nodes.forEach(el => {
		...
		el.classList.toggle('is-selected', selected[act]?.id && String(selected[act].id) === id);
	  });
	}
	...
	function renderAll(){
		...
		if (juntasAtiva) applyPageWinnerTint(null); // ← nunca tinte nas Juntas
      ...
      function renderAct(act){
		  ...
		  // tinte quando contagem concluída
		  const done = (apuradas > 0 && apuradas >= total);
		  const active = isTabActive(`tab-${act}`);
		  if (active && done){
			const votes = (row && (sel?.id || sel?.name))
			  ? { AD:N(row.votos_ad), PS:N(row.votos_ps), CHEGA:N(row.votos_chega), CDU:N(row.votos_cdu) }
			  : { AD:totals.ad, PS:totals.ps, CHEGA:totals.chega, CDU:totals.cdu };
			const winner = winnerFromVotes(votes);
			applyPageWinnerTint(winner ? PARTY_COLORS[winner] : null);
		  } else if (active){
